...
function ParseMapResources() {
    ...
    fs.readFile(w3tFilePath, function (err, data) {
        ...
        const deoptimizedJsonPath = path.resolve(mapBasePath + '/deoptimized/war3map.w3t.json');
        fs.writeFileSync(deoptimizedJsonPath, JSON.stringify(deoptimizedJson, null, 4));
    });
    ...
    fs.readFile(w3aFilePath, function (err, data) {
        ...
        // Recreated
        var warResult = new Translator.Objects.jsonToWar('abilities', jsonResult.json);
        const warPath = path.resolve(mapBasePath + '/recreated/war3map.w3a');
        fs.writeFileSync(warPath, warResult.buffer);

        // Deoptimized
        var deoptimizedJson = Deoptimize(jsonResult.json);
        const deoptimizedJsonPath = path.resolve(mapBasePath + '/deoptimized/war3map.w3a.json');
        fs.writeFileSync(deoptimizedJsonPath, JSON.stringify(deoptimizedJson, null, 4));
    });
    ...
    fs.readFile(w3uFilePath, function (err, data) {
        ...
        // Deoptimized
        var deoptimizedJson = Deoptimize(jsonResult.json);
        const deoptimizedJsonPath = path.resolve(mapBasePath + '/deoptimized/war3map.w3u.json');
        fs.writeFileSync(deoptimizedJsonPath, JSON.stringify(deoptimizedJson, null, 4));
    });
...
function Deoptimize(json) {
    ...
    for (let index = 0; index < keys.length; index++) {
        ...
        var fourccNum = To8Bit(fourcc1.charCodeAt(0)) * 256 * 256 * 256 
